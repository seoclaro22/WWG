...
interface GlowingShadowButtonProps {
  children: ReactNode
  // Tono fijo del brillo en grados (0-360). Por defecto dorado (~45deg), para
  // que encaje con la marca. Pasa `rainbow` para recuperar el ciclo de color
  // original del componente.
  hue?: number
  rainbow?: boolean
  // El diseno original asume una tarjeta grande (480px, 1.5:1). Para envolver
  // algo pequeno (un boton, un icono) hace falta poder ajustar ambos valores;
  // si no, el contenedor fuerza sus propias proporciones sobre el contenido.
  width?: string
  aspectRatio?: string
  // El radio original (3.6vw) da una tarjeta de esquinas redondeadas. Envolviendo
  // un boton circular se veia como un parche cuadrado detras; con estos dos se
  // puede dejar el marco redondo y sin fondo propio.
  radius?: string
  contentColor?: string
}
    ...
    <>
      <style jsx>{`
        ...
        .glow-content:before {
          ...
          animation: ${rainbow ? 'hue-animation var(--animation-speed) linear infinite, ' : ''}rotate-bg var(--animation-speed) linear infinite;
        ...
        .glow:after {
          ...
          position: relative;
          border-radius: calc(var(--glow-radius) * 10vw);
          ${rainbow ? 'animation: hue-animation var(--animation-speed) linear infinite;' : ''}
