...
import { djPath } from '@/lib/hrefs'

type Props = {
  dj: { id: string; slug?: string | null; name: string; name_i18n?: Record<string,string> | null; short_bio?: string | null; short_bio_i18n?: Record<string,string> | null; bio?: string | null; bio_i18n?: Record<string,string> | null; genres?: string[] | null; image?: string | null }
}
    ...
    <div className="card card-glass overflow-hidden relative">
      ...
      <div className="flex gap-3 p-3 items-start">
        ...
        <div className="flex-1">
          <Link href={ruta} className="font-medium leading-tight hover:text-gold block"><LocalText value={dj.name} i18n={dj.name_i18n||undefined} /></Link>
          {genres.length > 0 && (
          ...
          <div className="mt-2 flex gap-2">
            <Link href={ruta} className="btn btn-secondary text-sm px-3 py-1">{t('action.view')}</Link>
          </div>
        </div>
      </div>
    </div>
