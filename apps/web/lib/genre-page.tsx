// Implementacion compartida de /genre/[name] y /genero/[name].
//
// Mismo patron que lib/zone-genre-page.tsx: dos carpetas de ruta, una por
// segmento traducido (GENRE_SEGMENT en hrefs.ts), cada page.tsx llama a
// estas funciones pasando su propio segmento esperado.
...
type Params = { locale: string; name: string }
...
// la URL: en ambos casos la respuesta es un 308 a la forma canonica.
...
export async function genreMetadata(params: Params, expectedSegment: string) {
...
export async function renderGenrePage(params: Params, expectedSegment: string) {
  const resolved = await resolveGenre(params.name, params.locale, expectedSegment)
  if (!resolved) return notFound()
  const name = resolved.name
  if (resolved.redirect) permanentRedirect(localizedPath(genrePath(name, params.locale), params.locale))
          ...
          <div className="space-y-3 pt-2 border-t border-white/10">
            ...
            <div className="flex flex-wrap gap-2">
              {zonasConAgenda.map((z) => (
                <Link
                  key={z.slug}
                  href={zoneGenrePath(z.slug, name, params.locale)}
